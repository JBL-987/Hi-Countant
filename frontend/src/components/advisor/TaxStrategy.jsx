...
import { Receipt, AlertCircle, TrendingUp, ChevronRight, Loader } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gray-900 border border-blue-900/30 rounded-lg p-4 shadow-md">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-gray-400 text-sm font-medium">Taxable Income</h3>
            <Receipt className="h-5 w-5 text-green-500" />
          </div>
          <p className="text-2xl font-bold text-white">{taxData?.taxSummary?.taxableIncome}</p>
        </div>
        <div className="bg-gray-900 border border-blue-900/30 rounded-lg p-4 shadow-md">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-gray-400 text-sm font-medium">Total Deductions</h3>
            <TrendingUp className="h-5 w-5 text-blue-500" />
          </div>
          <p className="text-2xl font-bold text-white">{taxData?.taxSummary?.totalDeductions}</p>
        </div>
        <div className="bg-gray-900 border border-blue-900/30 rounded-lg p-4 shadow-md">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-gray-400 text-sm font-medium">Estimated Tax</h3>
            <AlertCircle className="h-5 w-5 text-red-500" />
          </div>
          <p className="text-2xl font-bold text-white">{taxData?.taxSummary?.estimatedTax}</p>
        </div>
      </div>

      {/* Deductions */}
      <div className="rounded-xl bg-gray-900 border border-blue-900/30 p-6 shadow-lg">
        <h2 className="mb-6 text-xl font-bold text-white">Tax Deductions</h2>
        <div className="space-y-4">
          {taxData?.deductions?.map((deduction) => (
            <div key={deduction.id} className="bg-gray-800 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium text-white">{deduction.category}</h3>
                <span className="text-sm text-gray-400">Percentage: {deduction.percentage}%</span>
              </div>
              <div className="mt-2">
                ...
                <div className="flex justify-between mt-1">
                  <span className="text-sm text-gray-400">Amount: {deduction.amount}</span>
                </div>
              </div>
            </div>
          ))}
